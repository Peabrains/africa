...
self.addEventListener('fetch', e => {
  ...
  // InstantDB / external APIs — network only (never cache auth data)
  ...
  // Open-Meteo weather — network first, no cache
  ...
  // Everything else — cache first, network fallback (works offline in the bush)
  e.respondWith(
    caches.match(e.request).then(cached => {
      if (cached) return cached;
      return fetch(e.request).then(res => {
