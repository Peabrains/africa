...
const PlannerScreen = (() => {
  ...
  function importView() {
    ...
    const actions = el('div', 'planner-actions'); const back = el('button', 'planner-secondary-action', 'Back to planner'); back.type = 'button'; back.addEventListener('click', () => { importReview = null; render(); }); const add = el('button', 'planner-primary-action', `Add ${importReview.items.filter(item => item.selected).length} selected to itinerary`); add.type = 'button'; add.addEventListener('click', () => { Toast.show('Approved items are ready to add ✓', 'success'); importReview = null; render(); }); actions.append(back, add); section.append(actions); return section;
  }
