...
Deno.serve(async req => {
  ...
  try {
    ...
    const response = await fetch("https://ai-gateway.vercel.sh/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${gatewayKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: Deno.env.get("AI_IMPORT_MODEL") || "openai/gpt-4.1-mini", max_output_tokens: 3000, input: [{ role: "developer", content: "Extract only travel itinerary facts visible in the image. Never invent missing dates, times, locations, booking references, or names. Classify each record as Day, Stop, Accommodation, or Flight. A Day record may contain only the day/date and a short destination label; it must never contain a list of activities in notes. Split every visible place, activity, meal, transport segment, or booking into its own Stop, Accommodation, or Flight record. If a day lists Morning, Midday, Afternoon, or Evening activities, create one separate record per activity. Do not collapse multiple places into one record. Use empty strings when a field is absent and Needs review when uncertain." }, { role: "user", content: [{ type: "input_text", text: `Extract this booking screenshot into itinerary records. Filename: ${String(fileName || "image")}` }, { type: "input_image", image_url: imageData }] }], text: { format: { type: "json_schema", name: "imported_itinerary", strict: true, schema: { type: "object", additionalProperties: false, required: ["items"], properties: { items: { type: "array", maxItems: 30, items: itemSchema } } } } } }) });
